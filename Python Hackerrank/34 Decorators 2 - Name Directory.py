...
#about N people. Each person has a first name, last name, age and sex. Print their
#names in a specific format sorted by their age in ascending order i.e. the youngest
#person's name should be printed first. For two people of the same age, print them
...
def person_lister(f):
    def inner(people):
        return [f(person) for person in sorted(people, key=lambda n: int(n[2]))]
    return inner
